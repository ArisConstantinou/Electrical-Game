...
In the hammer working stance, mouse and touch aiming automatically start with an independent gaze: up to **10° sideways / 7° vertically** leaves the head, arms and chisel work point stationary. Continuing past that range moves the working direction too. The crosshair follows the actual chisel tip on screen. Walking away or selecting another tool returns to normal aiming; no extra button is needed. This changes viewing direction only, so looking behind an obstruction still requires moving the head/body.
