...
const url='http://127.0.0.1:5366/Electrical-Game/';
...
if(occupied){
 for(const file of ['src/core/Game.ts','src/player/FPSRig.ts','src/player/WorkerBody.ts','src/electrical/BoxAssembly.ts']){
  ...
  if(!response.ok||typeof served!=='string'||normalize(served)!==normalize(await readFile(path.join(root,file),'utf8'))){
   throw new Error(`Port 5366 serves different source (${file}). Identify and stop that listener before restarting this preview. No process was stopped. Expected checkout: ${root}`);
  }
 }
 for(const file of ['public/assets/worker/worker.glb','public/assets/worker/skeleton.json']){
  ...
  if(!response.ok||hash(Buffer.from(await response.arrayBuffer()))!==hash(await readFile(path.join(root,file))))throw new Error(`Port 5366 serves a different worker asset (${file}). No process was stopped. Expected checkout: ${root}`);
 }
 ...
 const child=spawn(process.execPath,[vite,root,'--host','127.0.0.1','--port','5366','--strictPort'],{cwd:root,stdio:'inherit',windowsHide:true});
