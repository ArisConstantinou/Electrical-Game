...
const output = path.join(root, 'output/forge-hammer-depth', phase);
...
try {
  ...
  // Reorder draw calls on a completely frozen scene. Correct opaque geometry
  // must occlude itself regardless of batching/material sort order.
  await page.evaluate(() => {
    const g = window.__wireTheHouse, h = g.fpsRig.tools.get('hammer'); let i = 0;
    window.qaOrder = [];
    h.traverse(o => { if (o.isMesh) { window.qaOrder.push([o, o.renderOrder]); o.renderOrder = 1000 + i++; } });
  });
  await capture('order-a');
  await page.evaluate(() => window.qaOrder.forEach(([o], i) => { o.renderOrder = 2000 - i; }));
  await capture('order-b');
  await page.evaluate(() => window.qaOrder.forEach(([o, order]) => { o.renderOrder = order; }));
  if (phase !== 'before' && phase !== 'probe' && phase !== 'after') {
  ...
  if (phase === 'before') {
    await page.evaluate(() => window.__wireTheHouse.fpsRig.tools.get('hammer').traverse(o => {
      if (o.isMesh) for (const m of Array.isArray(o.material) ? o.material : [o.material]) { m.depthTest = true; m.depthWrite = true; m.needsUpdate = true; }
    }));
    await capture('depth-probe');
  }
  ...
  console.log(JSON.stringify({ output, errors, cases: report.cases.map(({ name, status, reachable, motorEyeM, motorNdc, drawCalls }) => ({ name, status, reachable, motorEyeM, motorNdc, drawCalls })) }, null, 2));
