...
export class MansionMasonryDemolition {
  ...
  constructor(
    ...
    private readonly bricks: THREE.InstancedMesh,
    ...
    this.remaining = new Uint8Array(bricks.count);
    for (let index = 0; index < bricks.count; index++) {
      bricks.getMatrixAt(index, this.temp);
  ...
  strike(index: number): boolean {
    ...
    this.bricks.setMatrixAt(index, this.temp);
  ...
  reset(): void {
    ...
    for (let index = 0; index < this.remaining.length; index++) {
      ...
      this.bricks.setMatrixAt(index, this.original[index]);
    }
    this.bricks.instanceMatrix.needsUpdate = true;
    this.bricks.computeBoundingSphere();
  ...
  private ensureMortarCells(): void {
    ...
    const cells = new THREE.InstancedMesh(geometry, this.backing.material, this.bricks.count);
    cells.name = `${this.group.name} remaining mortar cells`;
    cells.castShadow = cells.receiveShadow = true;
    for (let index = 0; index < this.bricks.count; index++) {
      this.original[index].decompose(this.position, this.rotation, this.scale);
