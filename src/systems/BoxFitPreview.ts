...
export class BoxFitPreview {
  ...
  mode='hidden';
  private pinned:{camera:THREE.Camera;layout:BoxModuleLayout[]}|null=null;
  private usingGuide=false;
  checks=0;
  lastCheckMs=0;
  private extraDepthMm=0;
  private proudDepthMm=0;
  ...
  constructor(scene:THREE.Scene,private readonly placement:BoxPlacementSystem){
    ...
    this.blockedMesh=new THREE.InstancedMesh(new THREE.PlaneGeometry(.0043,.0043),new THREE.MeshBasicMaterial({transparent:true,opacity:.62,depthTest:false,depthWrite:false,polygonOffset:true,polygonOffsetFactor:-2}),4096);
  ...
  pin(camera:THREE.Camera,preset:string|readonly BoxModuleLayout[]):void {
    const layout=typeof preset==='string'?horizontalBoxLayout(preset.split('+') as BoxKind[]):preset.map(module=>({...module}));
    if(this.assessment&&!this.assessment.fits&&['masonry','cured-mortar'].includes(this.assessment.reason)){this.pinned={camera:new THREE.Camera().copy(camera,false),layout};this.invalidate();}
  }
  clearGuide():void {this.pinned=null;this.invalidate();}
  get hasGuide():boolean {return this.pinned!==null;}
  update(camera:THREE.Camera,preset:string|readonly BoxModuleLayout[],enabled:boolean,retrieving:boolean,dt:number,canReach:(point:THREE.Vector3)=>boolean,guide=false,placedDepthM=0):void {
    let layout=typeof preset==='string'?horizontalBoxLayout(preset.split('+') as BoxKind[]):preset.map(module=>({...module}));
    this.usingGuide=guide&&this.pinned!==null;
    if(this.usingGuide){camera=this.pinned!.camera;layout=this.pinned!.layout;canReach=()=>true;retrieving=false;}
    this.elapsed+=dt;
    if(!enabled||retrieving){this.root.visible=false;this.mode=retrieving?'retrieve':'hidden';this.proudDepthMm=retrieving?Math.ceil(Math.max(0,placedDepthM)*1000):0;this.assessment=null;this.key='';return;}
    const layoutKey=boxAssemblyKey(layout),label=layout.map(module=>module.kind).join('+');
    const key=[layoutKey,...camera.position.toArray().map(n=>Math.round(n*2000)),...camera.quaternion.toArray().map(n=>Math.round(n*10000)),this.placement.fitRevision].join(':');
    if(key===this.key)return;
    if(this.key&&label===this.preset&&this.elapsed<.08)return;
    this.key=key;this.preset=label;this.elapsed=0;
    let ghost=this.ghosts.get(layoutKey);
    if(!ghost){ghost=new InstallationPoint({id:`preview-${this.ghosts.size+1}`,label:'Held box fit probe',kind:'socket',boxes:layout.map(module=>module.kind),boxLayout:layout.map(module=>({...module})),x:0,bottom:.3});this.ghosts.set(layoutKey,ghost);}
    ...
    const reachable=canReach(new THREE.Vector3(target.x,target.y,target.wallFrontZ+fit.seatDepthM));
    ...
    for(const box of ghost.boxGroup.boxes){
      const size=new THREE.Box3().setFromObject(box).getSize(new THREE.Vector3()),x=target.x+box.position.x,w=size.x/2,h=size.y/2,y=target.y+box.position.y,seat=target.wallFrontZ+fit.seatDepthM,z=seat+.007,back=seat-fit.required.depth;
    ...
    let attribute=this.outlines.geometry.getAttribute('position') as THREE.BufferAttribute;
    if(attribute.array.length<positions.length){let capacity=attribute.array.length;while(capacity<positions.length)capacity*=2;attribute=new THREE.BufferAttribute(new Float32Array(capacity),3).setUsage(THREE.DynamicDrawUsage);this.outlines.geometry.setAttribute('position',attribute);}
    ...
    (this.outlines.material as THREE.LineBasicMaterial).color.set(fit.reason==='out-of-reach'?0x69b8ff:fit.fits?(reachable?0x54ec93:0x69b8ff):fit.canPlace?0xffae32:0xff5948);
    let count=0;
  ...
  get telemetry(){const fit=this.assessment;return{mode:this.mode,guide:this.usingGuide,visible:this.root.visible,checks:this.checks,lastCheckMs:this.lastCheckMs,preset:this.preset,blockedCells:this.blockedMesh.count,fits:fit?.fits??false,canPlace:fit?.canPlace??false,reason:fit?.reason??null,target:fit?.target??null,required:fit?.required??null,extraDepthMm:fit?this.extraDepthMm:0,proudDepthMm:this.proudDepthMm,finishDepthMm:fit?Math.round(fit.seatDepthM*1000):0,backstopDepthMm:fit?Math.round(fit.backstopDepthM*1000):0};}
}
