...
export class MortarSlump {
  ...
  reset(seed:number):void{
    this.seed=seed>>>0;this.patches.length=0;this.displacement.set(0,0);this.charge=this.cooldown=0;this.revision++;
    for(let z=0;z<4;z++)for(let x=0;x<3;x++)this.patches.push({x:(x-1)*.19+(this.random()-.5)*.05,z:(z-1.5)*.20+(this.random()-.5)*.05,rx:.10+this.random()*.045,rz:.11+this.random()*.07,height:.024+this.random()*.022,grip:.095+this.random()*.105,rest:1,eroded:0,sliding:false,offset:new THREE.Vector2(),velocity:new THREE.Vector2()});
  ...
  update(dt:number,forceX:number,forceZ:number):void{
    ...
    for(const p of this.patches){
      ...
      const fx=forceX,fz=forceZ,stress=Math.hypot(fx,fz),threshold=p.grip*(p.sliding?.8:1)+p.rest*.035;
      if(stress>threshold){
    ...
    this.displacement.multiplyScalar(1/this.patches.length);this.cooldown=Math.max(0,this.cooldown-dt);
  }
  height(x:number,z:number):number{
    let h=0;
    for(const p of this.patches){
      const dx=(x-p.x-p.offset.x)/p.rx,dz=(z-p.z-p.offset.y)/p.rz,r=dx*dx+dz*dz;
      const sourceX=(x-p.x)/(p.rx*1.6),sourceZ=(z-p.z)/(p.rz*1.6);
      // A sliding cohesive lump piles up locally, leaving a broader shallow
      // trough. Different widths/heights break up a flat, water-like surface.
      const pile=p.offset.length()*.28;
      h+=(1-p.eroded)*(p.height*Math.exp(-r*1.2)+pile*(Math.exp(-r*1.8)-Math.exp(-(sourceX*sourceX+sourceZ*sourceZ)*1.8)/2.56));
    }
    // Fixed uneven trowel ridges are carried by the bulk; no time-based ripples.
    const px=x-this.displacement.x,pz=z-this.displacement.y;
    h+=.008*(Math.abs(Math.sin(px*51+Math.sin(pz*33))*Math.sin(pz*49+Math.cos(px*21)))-.4)+.002*Math.sin(px*67-pz*33);
    for(const [cx,cz,radius] of [[-.11,-.16,.14],[.10,.05,.17],[-.05,.26,.13]]){
      const radiusAtPoint=Math.hypot(px-cx,(pz-cz)*.85),edge=(radiusAtPoint-radius)/.014;
      h+=.016*Math.exp(-edge*edge)-.014*Math.exp(-Math.pow((radiusAtPoint-radius+.024)/.022,2));
    }
    return h;
  }
  shedAt(x:number,z:number,mass:number):void{
    ...
    p.eroded=Math.min(.95,p.eroded+mass/4);this.revision++;
  }
  ...
  get telemetry(){return{patches:this.patches.map(p=>({offset:p.offset.toArray(),speed:p.velocity.length(),sliding:p.sliding})),displacement:this.displacement.toArray()};}
}
