...
function spray(): THREE.Group {
  ...
  const info = label('COLOR', '400 ml Â· MASONRY', .058, .044); if (info) { info.position.set(0, -.005, .034); can.add(info); }
...
function cutter(): THREE.Group {
  ...
  const badge = label('PVC', 'Ã˜ 25', .027, .014, '#ffffff', '#b92028'); if (badge) { badge.position.set(.122, -.065, .012); badge.rotation.z = -.45; assembly.add(badge); }
