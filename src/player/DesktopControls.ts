...
export class DesktopControls {
  ...
  constructor(surface: HTMLElement, private readonly lockTarget: HTMLElement, player: PlayerController, input: Input) {
    ...
    surface.addEventListener('pointerdown', event => {
      ...
      if (event.button === 0 && document.pointerLockElement !== this.lockTarget) this.requestLock(false);
    ...
    addEventListener('keydown', event => {
      ...
      if (event.code === 'KeyV' && event.shiftKey && !event.repeat) window.dispatchEvent(new CustomEvent('wirehouse:cycle-spray-mode'));
      ...
      if (event.code === 'KeyT' && event.shiftKey && !event.repeat) window.dispatchEvent(new CustomEvent('wirehouse:cycle-chisel'));
