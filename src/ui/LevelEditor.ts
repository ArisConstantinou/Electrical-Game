...
export class LevelEditor {
  ...
  constructor(private readonly game: Game) {
    ...
    this.panel.innerHTML = `<header><div><small>WIRE THE HOUSE · LIVE SITE</small><h2>LEVEL EDITOR</h2></div><div class="level-editor__header-actions"><button id="level-settings" type="button" aria-label="Editor navigation settings">⚙</button><button id="level-close" type="button" aria-label="Close level editor">✕</button></div></header>
      ...
      <section class="level-editor__inspector"><b id="level-name">Select an element</b><p id="level-kind">Tap a structure in the scene or list.</p><div class="level-editor__history"><button id="level-undo" type="button">UNDO</button><button id="level-redo" type="button">REDO</button></div><div class="level-editor__fields"><label>X <input data-axis="x" type="number" step="0.01"></label><label>Y <input data-axis="y" type="number" step="0.01"></label><label>Z <input data-axis="z" type="number" step="0.01"></label><label>WIDTH m <input data-size="x" type="number" min="0.2" step="0.01"></label><label>HEIGHT m <input data-size="y" type="number" min="0.2" step="0.01"></label><label>DEPTH m <input data-size="z" type="number" min="0.05" step="0.01"></label><label>YAW ° <input id="level-yaw" type="number" step="1"></label></div><button id="level-remove-sill" type="button" hidden>REMOVE WINDOW SILL</button><button id="level-delete" type="button">DELETE ADDED ELEMENT</button><p id="level-status" role="status"></p></section>
