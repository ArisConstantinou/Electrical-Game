...
try{for(const mobile of [false,true]){
 ...
 await page.goto('http://127.0.0.1:5362/Electrical-Game/');await page.waitForFunction(()=>window.__wireTheHouse);
 ...
 await page.evaluate(()=>{const g=window.__wireTheHouse,c=g.renderer.camera;g.hammerWorkStance.restore(c);c.position.set(.7,g.player.eyeHeight,g.room.brickWall.volume.frontZ+.85);c.lookAt(.7,1.45,g.room.brickWall.volume.frontZ);g.player.yaw=c.rotation.y;g.player.pitch=c.rotation.x;for(let i=0;i<60;i++)window.__perfectStep(1/60);});
 ...
 await page.evaluate(()=>{for(let i=0;i<240;i++)window.__perfectStep(1/60);});
 const state=await page.evaluate(()=>{const g=window.__wireTheHouse;g.renderer.render();return{...g.mortar.telemetry,overflow:document.documentElement.scrollWidth>innerWidth};});
 assert(state.stuckKg>.60,`${mobile?'mobile':'desktop'}: correct dry wall cast retained only ${state.stuckKg}`);
 assert(state.floorKg<.05);assert(Math.abs(state.launchedKg-state.stuckKg-state.floorKg-state.restingKg-state.movingKg)<1e-6);assert(!state.overflow);assert.deepEqual(errors,[]);
 await page.screenshot({path:`${out}/${mobile?'mobile':'desktop'}.png`});report.push({mobile,phase,state,errors});await page.close();
