...
// Immutable current-state oracle for performance-only fragment extraction edits.
// Compare the same live wall, its exact cached-clipper facets, fragment vertices,
// saved material edits and worker-mesher inputs after every repeated impact.
const baselineRef = '26b3a84';
const output = 'output/masonry-impact-current-baseline';
const traceFacets = !process.argv.includes('--no-trace');
...
const report = { baselineRef, checks: [traceFacets ? 'exact impact state, canonical clipped facet boundaries and Float32 fragment vertex sets' : 'exact impact state and Float32 fragment vertex sets', 'exact persistent edits; removed mass within 1e-14 m3', 'exact deferred support results', 'exact mesher inputs'], cases: [] };
try {
  ...
  for (const seed of [1234, 193187, 8721]) for (const specimen of cases) {
    // Exercise the current rounded profile and the legacy, rendered and
    // concrete variants against the immutable pre-optimization implementation.
    const options = { seed, hollowProfile: 'rounded-five', ...specimen.options };
    const before = new Before(options), after = new After(options), times = [[], []], aggregates = [[], []], lookups = [0, 0];
    const traces=[[],[]];
    for(const [index,wall] of [before,after].entries()){
      if(traceFacets){
        const clip=wall.clipRemovedTetra;
        wall.clipRemovedTetra=(...args)=>{const poly=clip(...args);traces[index].push(faceKeys(poly.faces));return poly;};
      }
      const aggregate=wall.aggregateFragments;
      wall.aggregateFragments=(...args)=>{const started=performance.now();try{return aggregate.apply(wall,args);}finally{aggregates[index].push(performance.now()-started);}};
    }
    for (const [index, wall] of [before, after].entries()) {
    ...
    report.cases.push({ seed, name: specimen.name, fragments, baseline: summary(times[0]), current: summary(times[1]), aggregateBaseline: summary(aggregates[0]), aggregateCurrent: summary(aggregates[1]), nodeMaterialQueries: { baseline: lookups[0], current: lookups[1] } });
  }
  report.passed = true;
  report.traceFacets = traceFacets;
  report.total = {
    ...
    aggregateBaselineMs: report.cases.reduce((sum, item) => sum + item.aggregateBaseline.totalMs, 0),
    aggregateCurrentMs: report.cases.reduce((sum, item) => sum + item.aggregateCurrent.totalMs, 0),
  };
