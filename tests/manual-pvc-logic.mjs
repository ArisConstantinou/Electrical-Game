...
for(const mark of [.5,1.4,2.6]){
 ...
 for(let i=0;i<60;i++)b.press(1/60);assert.equal(b.angle,12,'One fixed hand position must saturate, never make an automatic full bend');
 for(let cell=1;cell<8;cell++){b.move(1);for(let i=0;i<30;i++)b.press(1/60);}
 assert(Math.abs(b.angle-90)<1e-6);assert(b.ready);assert(b.radius>.11,'Eight-position bend must retain at least a 110 mm centreline radius');
...
console.log('PASS: eight-position progressive bending, exact 90 degrees, conserved 3 m length, independent recipes, finite grip range');
