...
try{
 ...
 // Test recovery from a severe, directed loss of balance. Ordinary Shift
 // acceleration must no longer be used as the trigger (balance regression).
 for(const direction of ['KeyA','KeyD','KeyW','KeyS','diagonal']){
   await page.evaluate(direction=>{const g=window.__wireTheHouse,w=g.mixing.wheelbarrow;w.release();w.state='parked';w.pitch=w.roll=w.pitchSpeed=w.rollSpeed=0;w.tipExposure=0;w.mortarSlump.reset(3185);w.massKg=114;w.shovelKg=0;w.consumedKg=0;w.parcels.length=0;w.model.group.position.set(direction==='diagonal'?1.2:0,0,direction==='diagonal'?-.8:-.15);w.yaw=0;w.model.group.rotation.set(0,0,0);},direction);
   await aim('cart');await key('KeyE');await step(80);
   await page.evaluate(direction=>{const w=window.__wireTheHouse.mixing.wheelbarrow;w.rollSpeed=direction==='KeyA'?9:direction==='KeyD'?-9:direction==='diagonal'?7:0;w.pitchSpeed=direction==='KeyW'?10:direction==='KeyS'?-10:direction==='diagonal'?7:0;},direction);await step(200);
   const flipped=await state();report.cases.push({name:direction,after:flipped,pattern:await page.evaluate(()=>window.__wireTheHouse.mixing.wheelbarrow.parcels.filter(p=>p.mass>0).map(p=>({mass:p.mass,position:p.position.toArray(),scale:p.scale.toArray()})))});await shot(`flip-${direction}`);assert.equal(flipped.state,'flipped',`${direction} severe disturbance can tip the cart`);assert(Math.abs(flipped.totalKg-114)<1e-6,'Flip conserves all mortar');
