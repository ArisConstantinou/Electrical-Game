...
const motion=true; const phase=process.argv.includes('--before')?'before':'after';
try{
 ...
 if(phase==='before')await page.route('**/assets/worker/worker.glb',async r=>r.fulfill({body:await readFile('output/outfit-correction/before/worker.glb'),contentType:'model/gltf-binary'}));await page.goto('http://127.0.0.1:5365/Electrical-Game/?renderer=webgl');await page.waitForFunction(()=>window.__wireTheHouse?.workerBody.loaded,{},{timeout:90000});await page.locator('#start-button').click();
 ...
 },motion);await writeFile('output/outfit-correction/'+phase+'/performance.json',JSON.stringify(result,null,2));console.log(JSON.stringify(result));
