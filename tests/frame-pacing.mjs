...
for(const hz of [30,60,120,144,240])for(const limit of [15,30,60,120,0]){
...
for(const saved of [null,'bad','Infinity','-1']){globalThis.localStorage={getItem:()=>saved};assert.equal(readFrameRateLimit(),60);}
for(const saved of ['0','30','60','120']){globalThis.localStorage={getItem:()=>saved};assert.equal(readFrameRateLimit(),Number(saved));}
globalThis.localStorage={getItem(){throw Error('blocked');}};assert.equal(readFrameRateLimit(),60);
console.log('PASS: 25 refresh/cap combinations, stall/reset, missing/corrupt/blocked storage.');
