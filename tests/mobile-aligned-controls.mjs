...
try{for(const viewport of [{width:390,height:844},{width:844,height:390},{width:320,height:740},{width:667,height:375},{width:820,height:1180},{width:1024,height:768}]){
 ...
 await page.evaluate(()=>{const g=window.__wireTheHouse,cart=g.mixing.wheelbarrow,root=cart.model.group;g.apprentice.command('cancel');g.mixing.setActive(false);g.player.velocity.set(0,0,0);g.renderer.camera.position.copy(root.localToWorld(g.renderer.camera.position.clone().set(0,1.65,-1.1)));g.player.yaw=root.rotation.y+Math.PI;g.player.pitch=-.60;});await step(3);
 const cartAim=await page.evaluate(()=>{const g=window.__wireTheHouse;return{target:g.mixing.telemetry.aimedTarget,apprentice:g.apprentice.mode};});assert.equal(cartAim.apprentice,'off','Exit apprentice commands before manual cart interaction');assert.equal(cartAim.target,'wheelbarrow','Aim directly at the cart before INTERACT');
