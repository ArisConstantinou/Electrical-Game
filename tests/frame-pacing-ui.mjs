...
const report={base,baseline,viewport:{width:1600,height:900},cases:[],errors:[]};
const browser=await chromium.launch({channel:'chrome',headless:false});
try{
 const context=await browser.newContext({viewport:report.viewport,deviceScaleFactor:1});
 await blockPointerLock(context);
 const page=await context.newPage();page.on('pageerror',e=>report.errors.push(String(e)));
 if(process.env.QA_DIST){
  const dist=path.resolve(process.env.QA_DIST),prefix='/Electrical-Game/';
  await page.route('http://127.0.0.1:5365/Electrical-Game/**',async route=>{
   const u=new URL(route.request().url()),relative=decodeURIComponent(u.pathname.slice(prefix.length))||'index.html';
   const file=path.resolve(dist,relative);
   if(file!==dist&&!file.startsWith(dist+path.sep))throw Error('Asset escaped candidate dist');
   try{await access(file);await route.fulfill({path:file});}
   catch{report.errors.push(`Missing candidate asset: ${relative}`);await route.fulfill({status:404,body:'Missing candidate asset'});}
  });
 }
 ...
 for(const limit of ['30','60','120','0']){
  ...
  await page.evaluate(()=>{const g=window.__wireTheHouse;g.renderer.camera.position.set(-1.72,g.player.eyeHeight,-.58);g.player.velocity.set(0,0,0);});
 ...
 const water=await page.evaluate(async()=>{const g=window.__wireTheHouse;g.apprentice.command('cancel');g.selectTool('hose');const before=g.mortar.waterGunLitres;g.input.actionHeld=true;try{await new Promise(r=>setTimeout(r,1000));}finally{g.input.actionHeld=false;}return{before,after:g.mortar.waterGunLitres,error:g.renderer.renderError};});
 ...
 assert.equal(await page.locator('#mobile-tool-slider').isVisible(),false,'Mobile tool strip stays behind settings');
