...
try{
 ...
 for(const[dx,dy]of targets){loads.push(await page.evaluate(({dx,dy,oblique})=>{const q=window.__mortarVolumeQA,{m,c,v}=q;c.position.set(.55+dx,1.4+dy,v.frontZ+.4);c.lookAt(.55+dx,1.4+dy,v.frontZ-.06);c.updateMatrixWorld(true);let hit=m.contact(c.position,c.getWorldDirection(c.position.clone().set(0,0,0)),.9);if(hit)m.applyWater(hit.point,hit.normal,.035);let packed;
 if(oblique&&hit){const origin=hit.point.clone().set(.55+dx+(dx?Math.sign(dx)*.08:0),1.4+dy+(dy?Math.sign(dy)*.08:0),v.frontZ+.4),time=.18,velocity=hit.point.clone().sub(origin).multiplyScalar(1/time);velocity.y+=4.905*time;m.launch(origin,velocity);packed=true;}else packed=m.pack(c);
 for(let i=0;i<100;i++)m.update(.01);return{packed,hit:hit?{point:{...hit.point},normal:{...hit.normal}}:null,coverage:m.coverage(q.point),telemetry:m.telemetry};},{dx,dy,oblique}));}
 await page.evaluate(()=>{for(let i=0;i<200;i++)window.__mortarVolumeQA.m.update(.01);});await capture('02-four-loads-continuous');
 ...
 assert(final.ready,'Actual masonry single gang not ready in four loads');
 ...
 const report={application:oblique?'four oblique ballistic launches':'four direct pack actions',fixture:'Production hollow MasonryVolume excavated with real impact API and production ElectricalBox. Camera/fixture placement and direct public application API are diagnostic, not normal-control gameplay proof.',excavation,loads,final,wash,errors};
 await writeFile(`${out}/report.json`,JSON.stringify(report,null,2));console.log(JSON.stringify(report,null,2));if(!oblique)assert(final.ready,'Actual masonry single gang not ready in four loads');assert.deepEqual(errors,[]);
