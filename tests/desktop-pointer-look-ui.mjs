...
try {
  ...
  assert.equal(started.locked, null, 'Starting the game must leave the PC pointer free');
  ...
  assert(Math.abs(moved.yaw - started.yaw) > .02, `Free pointer did not turn the camera: ${JSON.stringify({ started, moved })}`);
  assert.equal(moved.locked, null, 'Mouse look must not capture the pointer');
  await page.locator('#settings-toggle').click();
  assert.equal(await page.locator('#settings-toggle').getAttribute('aria-expanded'), 'true');
  assert.equal((await state()).locked, null, 'Settings must remain reachable with a free pointer');
  await page.locator('#settings-close').click();
  await page.locator('#game-canvas').click({ position: { x: 400, y: 450 } });
  assert.equal((await state()).locked, null, 'Clicking the playfield must not lock the pointer');
