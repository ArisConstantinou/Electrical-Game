...
try{for(const viewport of[{width:390,height:844},{width:844,height:390},{width:820,height:1180},{width:1180,height:820}]){
 ...
 await openTools();const cancelTool=await center('#mobile-tool-slider [data-tool="fitting"]');await start(9,cancelTool.x,cancelTool.y);await cancel();await step(p,1);assert.equal((await state(p)).selected,'hammer','Canceled tool tap selected a tool');
 // Click and keyboard paths remain usable after touch compatibility suppression.
 await p.locator('#mobile-tool-slider [data-tool="spray"]').click();await step(p,1);assert.equal((await state(p)).selected,'spray');await openTools();await p.locator('#mobile-tool-slider [data-tool="hammer"]').focus();await p.keyboard.press('Enter');await step(p,1);assert.equal((await state(p)).selected,'hammer');
 report.cases.push({platform,phase:'toolbelt-pan-tap',beltBefore,beltAfter});
 // Swiping an ordinary play area never fires, including old saved modes.
 for(const mode of['manual','auto-use','double-tap'])for(const tool of['hammer','spray','hose','trowel','fitting']){
  await select(tool);await place();await p.evaluate(mode=>window.__wireTheHouse.mobileControls.setAimControlMode(mode),mode);const before=await state(p);await start(1,free.x,free.y);await move(1,free.x+24,free.y-5);const during=await state(p);assert(!during.held&&!during.requested,`${platform}/${mode}/${tool}: free look fired`);await step(p,12);await end(1);await step(p,60);const after=await state(p);assert(!after.held&&!after.requested);assert.equal(after.strikes,before.strikes);assert.equal(after.mass,before.mass);assert.equal(after.water,before.water);assert.equal(after.marks,before.marks);assert(Math.abs(after.yaw-before.yaw)>.001,'Free look did not aim');report.cases.push({platform,phase:'look-only',mode,tool,before,after});
 }
 await select('hammer');await place();let pad=await center('#site-pro-use');
 ...
 for(const kind of['cancel','lost-capture','blur','settings','tool-switch','resize']){
  ...
  if(kind==='cancel')await cancel();
  if(kind==='lost-capture')await move(2,pad.x+1,pad.y);
  ...
  if(kind==='resize')await p.setViewportSize(viewport);
  report.cases.push({platform,phase:'cancel',kind});
 }
 await select('hammer');await place();await p.evaluate(async()=>{const g=window.__wireTheHouse;await g.renderer.waitForFrame();window.__manualStep(0);await g.renderer.waitForFrame();});await p.screenshot({path:`${out}/${platform}.png`});const final=await state(p);assert.equal(final.pointerLock,null);assert.equal(final.renderError,'');await context.close();console.log(`${platform}: PASS manual look, explicit USE, independent touch ownership and cancellation`);
