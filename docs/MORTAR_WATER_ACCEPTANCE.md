...
Production verification exposed an asynchronous import cycle that the development module graph did not reproduce: the entry module awaited water initialization while the lazy water chunk imported shared Three.js exports from that entry bundle. Startup now attaches its ready callback without a top-level await, allowing the entry module to finish evaluation. `tests/production-startup.mjs` exercises unmodified built files on the existing local origin, and can also target published HTTPS assets without routing. Both WebGPU desktop and WebGL2 mobile-sized scenarios verify startup, brake, hammer impacts, hose water reaching the floor and completed rendering. No second development server is needed.
