...
The user's already-open normal Chrome tab was inspected without reloading or entering gameplay. It was ready at the START menu and responsive; a three-second CDP sample measured 13.0% renderer main-thread busy time and about 93 MB used JS heap. The captured console had Three.js geometry/fog warnings and one browser-extension listener error, but no game renderer error. None of these explain the loading freeze. This is a post-load menu observation, not reproduction of the failing navigation.
